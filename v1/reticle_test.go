...
func TestMeasurement_HTTP(t *testing.T) {
	t.Log("HTTP should return a HTTP struct from the Raw field of the Measurement or throw error if the measurement is not a HTTP measurement")
	...
	for _, test :=range parseTest {
		...
		if m.Type != "http" && err == nil {
			t.Logf("\tGetting a http from type %s should result in error, but didn't %s", m.Type, crossMark)
			t.Fail()
		}

		if m.Type == d.Type && err == nil {
			t.Logf("\tReturning HTTP %s", checkMark)
		}
	}
}